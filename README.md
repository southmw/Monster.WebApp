...
2. **데이터베이스 설정**
   ...
   HTTPS를 사용할 수 없는 호스팅(예: MonsterASP 무료 플랜)에 배포하는 경우, 아래 옵션을 추가해야 HTTP에서도 로그인 쿠키가 저장됩니다 (미설정 시 프로덕션은 Secure 쿠키 강제 — HTTP 접속에서는 브라우저가 쿠키를 저장하지 않아 로그인이 유지되지 않음). **HTTPS 가능한 환경에서는 이 설정을 제거하세요** (인증 쿠키가 평문으로 전송될 수 있습니다):
   ```json
   {
     "Auth": {
   ...
   리버스 프록시/로드밸런서 뒤에 배포하는 경우, 클라이언트 IP 식별(로그인 잠금·추천 중복 방지)을 위해 신뢰할 프록시 IP를 지정합니다 (미지정 시 X-Forwarded-For 헤더는 무시됨):
   ```json
   {
     "ForwardedHeaders": {
